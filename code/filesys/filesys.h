...
class FileSystem {
  public:
    ...
    FileSystem() {
        ...
        // openFileName[index] = "stdin";
        // openFile[index++] = this->Open("stdin");
        // openFileName[index] = "stdout";
    ...
    OpenFile* Open(char *name,int type = 0){
		...
		// int Open(char* file_name,int flag){
		// 	int freeIndex = -1;
		...
		// 		return -1;
		// 	}
		// 	//chi xu ly openMode loai 1 va 2
			
		// 	fileDescriptor = OpenForReadWrite(file_name, FALSE);
	...
	bool isOpen(char* filename) {
        for(int i = 2; i < FILE_MAX; i++) {
            if(openFileName[i] != NULL && !strcmp(filename, openFileName[i])) {
                return true;
                // bool open = true;
                // for(int j = 0; j < strlen(filename); i++) {
                //     if (filename[j] != openFileName[i][j]) {
                //         open = false;
                //         break;
                //     }
                // }
